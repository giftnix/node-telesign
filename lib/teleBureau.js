...
(function() {
  ...
  TeleBureau.submitEvent = function(options, callback) {
    if (!options || typeof options !== 'object') {
      ...
      throw new Error('Error calling TeleBureau Submit - "occurredAt" not provided in the request params.');
    } else if (!callback || typeof callback !== 'function') {
    ...
    options.occurredAt = moment(options.occurredAt).format(dateFormat);
    if (options.discoveredAt) options.discoveredAt = moment.utc(options.discoveredAt).format(dateFormat);
    if (options.verifiedAt) options.verifiedAt = moment.utc(options.verifiedAt).format(dateFormat);

    console.log(options.verifiedAt, options.discoveredAt);
